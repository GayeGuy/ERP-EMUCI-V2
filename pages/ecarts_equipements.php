<?php
// ============================================================
//  pages/ecarts_equipements.php — Historique des équipements manquants
//  Miroir de pages/ecarts_rivets.php / ecarts_pmma.php, adapté au modèle
//  de présence (pas de quantité système/physique à afficher).
// ============================================================
...
require_auth();
require_permission('ecarts_equipements', 'can_read');

$user      = current_user();
$role_slug = $user['role_slug'] ?? '';

$page_title  = 'Écarts équipements';
$active_page = 'ecarts_equipements';
...
$where  = ["ee.date_constat BETWEEN ? AND ?"];
$params = [$f_date_debut, $f_date_fin];

if ($f_statut) { $where[] = "ee.statut = ?"; $params[] = $f_statut; }
if ($f_site)   { $where[] = "ee.site_id = ?"; $params[] = $f_site; }
if ($f_source) { $where[] = "ee.source = ?"; $params[] = $f_source; }

$sql_where = 'WHERE ' . implode(' AND ', $where);

$ecarts = db_fetch_all(
    "SELECT ee.id, ee.date_constat, ee.motif, ee.source, ee.statut, ee.resolu_at, ee.created_at,
            e.numero_serie_interne, n.libelle AS type_libelle,
            s.nom AS site_nom,
     ...
     JOIN equipements e ON e.id = ee.equipement_id
     LEFT JOIN nomenclatures n ON n.id = e.nomenclature_id
     LEFT JOIN users ur ON ur.id = ee.resolu_par
     LEFT JOIN users uc ON uc.id = ee.created_by
     $sql_where
     ORDER BY ee.date_constat DESC, ee.created_at DESC",
    $params
);
...
<div class="ecarts-page">
  ...
  <div class="ecarts-header">
    ...
    <div>
      <h2>Écarts équipements</h2>
      <p style="margin:0;font-size:13px;color:var(--muted)">Équipements signalés manquants lors des inventaires — non retrouvés depuis</p>
    </div>
  </div>
  ...
  <div class="ecarts-kpis">
    <div class="kpi-card">
      ...
      <div>
        ...
        <div class="kpi-lbl">Toujours manquants</div>
      </div>
    </div>
    <div class="kpi-card">
      ...
      <div>
        ...
        <div class="kpi-lbl">Retrouvés</div>
      </div>
    </div>
    <div class="kpi-card">
  ...
  <form method="get" class="filtre-bar">
    ...
    <div>
      ...
      <select name="statut">
        ...
        <option value="ouvert"  <?= $f_statut === 'ouvert'  ? 'selected' : '' ?>>Toujours manquants</option>
        <option value="resolu"  <?= $f_statut === 'resolu'  ? 'selected' : '' ?>>Retrouvés</option>
      </select>
    </div>
    <div>
      <label>Source</label>
  ...
  <div class="table-wrap">
    ...
    <?php if (empty($ecarts)): ?>
      <div class="empty-state">
        ...
        Aucun équipement manquant pour cette période et ces filtres.
      </div>
    <?php else: ?>
      ...
      <table class="ecarts-table">
        ...
        <?php foreach ($ecarts as $e): ?>
          <tr>
            ...
            <td><?= h($e['site_nom']) ?></td>
            <td style="font-family:monospace;font-weight:800;color:var(--navy)"><?= h($e['numero_serie_interne']) ?></td>
            <td style="color:var(--muted);font-size:12px"><?= h($e['type_libelle'] ?: '—') ?></td>
            <td style="font-size:12px"><?= h($source_labels[$e['source']] ?? $e['source']) ?></td>
            <td>
              <?php if ($e['statut'] === 'ouvert'): ?>
                <span class="badge badge-ouvert">Toujours manquant</span>
              <?php else: ?>
                <span class="badge badge-resolu">Retrouvé</span>
              <?php endif; ?>
            </td>
