        ...
        try {
            ...
            db_query(
                "INSERT INTO stock_consommables_site (consommable_id,site_id,quantite)
                 VALUES (?,?,?) ON DUPLICATE KEY UPDATE quantite = quantite + VALUES(quantite)",
                [$conso_id, $site_id, $qte]
            );
    ...
    if ($action === 'ajustement') {
        ...
        try {
            db_query("INSERT INTO stock_consommables_site (consommable_id,site_id,quantite) VALUES (?,?,?)
                      ON DUPLICATE KEY UPDATE quantite = VALUES(quantite)", [$conso_id, $site_id, $new_qte]);
...
$kpi_receptions_mois = (float)db_fetch_value("SELECT COALESCE(SUM(quantite),0) FROM receptions_consommables WHERE date_reception >= DATE_FORMAT(CURRENT_DATE,'%Y-%m-01')");
$kpi_distrib_mois    = (float)db_fetch_value("SELECT COALESCE(SUM(quantite),0) FROM livraisons_consommables WHERE type_mouvement='distribution' AND date_livraison >= DATE_FORMAT(CURRENT_DATE,'%Y-%m-01')");
...
.tab-btn{padding:12px 20px;font-size:13.5px;font-weight:500;color:var(--muted);cursor:pointer;border-bottom:2px solid transparent;background:none;border-top:none;border-left:none;border-right:none;transition:all .15s;font-family:'DM Sans',sans-serif;white-space:nowrap}
