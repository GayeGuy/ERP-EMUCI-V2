<?php
// ============================================================
//  pages/ecarts_pmma.php — Historique des écarts PMMA
//  Miroir de pages/ecarts_rivets.php. type_pmma est un texte libre,
//  affiché tel quel (pas de table de libellés fixes).
// ============================================================
...
require_auth();
require_permission('ecarts_pmma', 'can_read');

$user      = current_user();
$role_slug = $user['role_slug'] ?? '';

$page_title  = 'Écarts PMMA';
$active_page = 'ecarts_pmma';
...
$where  = ["ep.date_constat BETWEEN ? AND ?"];
$params = [$f_date_debut, $f_date_fin];

if ($f_statut) { $where[] = "ep.statut = ?"; $params[] = $f_statut; }
if ($f_site)   { $where[] = "ep.site_id = ?"; $params[] = $f_site; }
if ($f_source) { $where[] = "ep.source = ?"; $params[] = $f_source; }

$sql_where = 'WHERE ' . implode(' AND ', $where);

$ecarts = db_fetch_all(
    "SELECT ep.id, ep.date_constat, ep.stock_systeme, ep.stock_physique, ep.ecart,
            ep.motif, ep.source, ep.statut, ep.resolu_at, ep.type_pmma, ep.created_at,
            s.nom AS site_nom,
            CONCAT(ur.prenom,' ',ur.nom) AS resolu_par_nom,
            CONCAT(uc.prenom,' ',uc.nom) AS created_by_nom
     FROM ecarts_pmma ep
     JOIN sites s ON s.id = ep.site_id
     LEFT JOIN users ur ON ur.id = ep.resolu_par
     LEFT JOIN users uc ON uc.id = ep.created_by
     $sql_where
     ORDER BY ep.date_constat DESC, ep.created_at DESC",
    $params
);
...
<div class="ecarts-page">
  ...
  <div class="ecarts-header">
    ...
    <div>
      <h2>Écarts PMMA</h2>
      <p style="margin:0;font-size:13px;color:var(--muted)">Historique des écarts détectés lors des inventaires PMMA</p>
    </div>
  </div>
      ...
      <table class="ecarts-table">
        ...
        <?php foreach ($ecarts as $e): ?>
          ...
          <tr>
            ...
            <td><?= h($e['site_nom']) ?></td>
            <td style="font-weight:700;color:var(--navy)"><?= h($e['type_pmma']) ?></td>
