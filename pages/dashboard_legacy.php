...
if ($role_slug === 'coordinateur_site' && $site_force) {
    ...
    $fin_cycle_list = db_fetch_all("SELECT e.numero_serie_interne,e.date_fin_cycle,e.etat,n.libelle AS type_equip,DATEDIFF(e.date_fin_cycle, CURRENT_DATE) AS jours_restants FROM equipements e JOIN nomenclatures n ON n.id=e.nomenclature_id WHERE e.actif=1 AND e.site_id=? AND e.date_fin_cycle IS NOT NULL AND e.date_fin_cycle<=(CURRENT_DATE + INTERVAL 60 DAY) ORDER BY e.date_fin_cycle ASC LIMIT 10", [$sid]);
    ...
    $interv_par_site  = db_fetch_all("SELECT s.nom AS site_nom, COUNT(im.id) AS nb_interv FROM sites s LEFT JOIN interventions_maintenance im ON im.site_id=s.id AND YEAR(im.date_intervention)=YEAR(CURRENT_DATE) AND MONTH(im.date_intervention)=MONTH(CURRENT_DATE) WHERE s.actif=1 AND s.id=? GROUP BY s.id", [$sid]);
...
// ── MAINTENANCE INFORMATIQUE : tout filtré catégorie=informatique
    ...
    $sites_par_type  = db_fetch_all("SELECT type, COUNT(*) AS nb FROM sites WHERE actif=1 GROUP BY type ORDER BY nb DESC");
    $fin_cycle_list  = db_fetch_all("SELECT e.numero_serie_interne,e.date_fin_cycle,e.etat,n.libelle AS type_equip,s.nom AS site_nom,DATEDIFF(e.date_fin_cycle, CURRENT_DATE) AS jours_restants FROM equipements e JOIN nomenclatures n ON n.id=e.nomenclature_id LEFT JOIN sites s ON s.id=e.site_id WHERE e.actif=1 AND e.categorie='informatique' AND e.date_fin_cycle IS NOT NULL AND e.date_fin_cycle<=(CURRENT_DATE + INTERVAL 60 DAY) ORDER BY e.date_fin_cycle ASC LIMIT 10");
    $conso_fcfa_site = [];
    $stock_bas_list  = [];
...
// ── SUPERVISEUR OPÉRATION : vue centrée sur les actions coordinateurs
    ...
    $equip_par_type  = [];
    $sites_par_type  = db_fetch_all("SELECT type, COUNT(*) AS nb FROM sites WHERE actif=1 GROUP BY type ORDER BY nb DESC");
    $interv_par_site = db_fetch_all("SELECT s.nom AS site_nom, COUNT(im.id) AS nb_interv FROM sites s LEFT JOIN interventions_maintenance im ON im.site_id=s.id AND YEAR(im.date_intervention)=YEAR(CURRENT_DATE) AND MONTH(im.date_intervention)=MONTH(CURRENT_DATE) WHERE s.actif=1 GROUP BY s.id ORDER BY nb_interv DESC LIMIT 10");
...
// ── ADMIN / GESTIONNAIRE / SUPERADMIN : vue globale complète
    ...
    $fin_cycle_list  = db_fetch_all("SELECT e.numero_serie_interne,e.date_fin_cycle,e.etat,n.libelle AS type_equip,s.nom AS site_nom,CONCAT(u.prenom,' ',u.nom) AS utilisateur,DATEDIFF(e.date_fin_cycle, CURRENT_DATE) AS jours_restants FROM equipements e JOIN nomenclatures n ON n.id=e.nomenclature_id LEFT JOIN sites s ON s.id=e.site_id LEFT JOIN users u ON u.id=e.utilisateur_id WHERE e.actif=1 AND e.date_fin_cycle IS NOT NULL AND e.date_fin_cycle<=(CURRENT_DATE + INTERVAL 60 DAY) ORDER BY e.date_fin_cycle ASC LIMIT 10");
