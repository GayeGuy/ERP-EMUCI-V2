...
$conso_mois  = (int)db_fetch_value("SELECT COALESCE(SUM(cb.quantite),0) FROM consommations_bobines cb JOIN op_bobines b ON b.id=cb.bobine_id WHERE b.serie IN ($serie_ph) AND YEAR(cb.date_conso)=YEAR(CURRENT_DATE) AND MONTH(cb.date_conso)=MONTH(CURRENT_DATE) $conso_today_where", $series_actives);
