...
foreach ([
    ...
    'distributions_site'  => 'en_cours_livraison',
...
// ── op_points_journaliers — diagnostic colonne type_point ────────
try {
    $col_tp = $db->query("SELECT COLUMN_TYPE, IS_NULLABLE, COLUMN_DEFAULT FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='op_points_journaliers' AND COLUMN_NAME='type_point'")->fetch(PDO::FETCH_ASSOC);
    $results[] = "op_points_journaliers.type_point ACTUEL : " . ($col_tp ? $col_tp['COLUMN_TYPE'].' NULL='.$col_tp['IS_NULLABLE'].' DEF='.$col_tp['COLUMN_DEFAULT'] : 'introuvable');
} catch(Exception $e){ $results[] = "diagnostic type_point : ❌ ".$e->getMessage(); }
...
try {
    $col_tp2 = $db->query("SELECT COLUMN_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='op_points_journaliers' AND COLUMN_NAME='type_point'")->fetchColumn();
    $results[] = "op_points_journaliers.type_point APRÈS : " . ($col_tp2 ?: 'introuvable');
} catch(Exception $e){ $results[] = "diagnostic après : ❌ ".$e->getMessage(); }
